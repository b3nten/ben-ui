import { type ElementType, type ReactNode, use, useMemo } from "react";
import {
	Box,
	type BoxProps,
	StyleContext,
...
export let Textbox = <T extends ElementType = "input">(
	...
	let compiledInputStyles = useCache(
		"textboxStyles",
		() => ({
			position: "relative",
			color: makeColor({
	...
	return withTooltip(
		<Box as={as ?? "input"} {...restProps} className={classes} css={styles} />,
		tooltip,
		color,
	);
};
...
export let Textarea = <T extends ElementType = "textarea">(
	...
	console.log(
		makeTransition({ target: "border-color" }, { target: "background" }),
	);
